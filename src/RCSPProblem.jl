...
function build_solvers!(model::T) where {T <: AbstractVrpModel}
    ...
    for prob in model.rcsp_instances
        prob.solver = @try_ccall(
            (:createAndPrepareSolver_c, rcsp_path), Ptr{Cvoid}, (Ptr{Cvoid}, Ptr{Cvoid}),
...
function run_rcsp_pricing(
    ...
    @try_ccall(
        (:getOutputPaths_c, rcsp_path), Cvoid, (Ptr{Cvoid}, Ptr{Cint}, Ptr{Cint}),
...
function run_rcsp_rcostfix_and_enum(
    ...
    return (
        @try_ccall(
            (:runRedCostFixingAndEnum_c, rcsp_path), Cint,
...
function record_rcsp_state(rcsp::RCSPProblem)
    ...
    return @try_ccall(
        (:recordPricingState_c, rcsp_path), Ptr{Cvoid}, (Ptr{Cvoid},), rcsp.solver,
    )
end
...
function restore_rcsp_state(solver::Ptr{Cvoid}, state::Ptr{Cvoid})
    ...
    @try_ccall(
        (:restorePricingState_c, rcsp_path), Cvoid, (Ptr{Cvoid}, Ptr{Cvoid}), solver, state,
    )
    return
end
...
function release_rcsp_state(state::Ptr{Cvoid})
    ...
    @try_ccall(
        (:releasePricingState_c, rcsp_path), Cvoid, (Ptr{Cvoid},), state,
    )
    return
end
...
function check_enumerated_paths(rcsp::RCSPProblem, paths::Vector{PathVarData})
    ...
    @try_ccall(
        (:checkEnumeratedPaths_c, rcsp_path), Cvoid,
...
function get_number_of_enum_paths(rcsp::RCSPProblem)
    return Int(@try_ccall(
        (:getNumberOfEnumPaths_c, rcsp_path), Cint, (Ptr{Cvoid},), rcsp.solver,
    ))
end
...
function get_enumerated_paths(rcsp::RCSPProblem)
    ...
    resize!(starts, nb_paths + 1)
    @try_ccall(
        (:getEnumeratedPaths_c, rcsp_path), Cint, (Ptr{Cvoid}, Cint, Ptr{Cint}, Cint, Ptr{Cint}),
